...
ini_set('max_execution_time', 600); // bulletproof solution for bulk import with synchronous adapter - Ping 2016
...
class CsvImport_Import extends Omeka_Record_AbstractRecord
{
    ...
    protected function _addItemFromRow($row)
    {
        ...
        /**
          * Edited by Ping Charoenwet - 2016 - 2017
          * url is replaced by amount of files in Files column
          *
          **/

         // adjust fileUrls from amount of files to real file names
         // in format ITXxxxxxxxxxxx.x.jpg        
        if(count($fileUrls) > 0 && is_numeric($fileUrls[0])) {
            
            $item_prefix = 'ITC'.$row['Dublin Core:Identifier']; // identifier is in column Dublin Core:Identifier : hard code
            $file_count = $fileUrls[0];
            $fileUrls = array();
            for($i=1;$i<=$file_count;$i++)
                array_push($fileUrls, $item_prefix.'.'.$i.'.jpg');
        ...
        if (!defined('CSV_IMPORT_PATH'))
            define('CSV_IMPORT_PATH', realpath(__DIR__ . '/../..'));
        
        foreach ($fileUrls as $url) {
            try {
        
                $files_dir = CSV_IMPORT_PATH . DIRECTORY_SEPARATOR . 'upload_files';
                
                $filePath = $files_dir .DIRECTORY_SEPARATOR . $url;

                $file = insert_files_for_item($item, 
                                                //'Url',    // change Url import to Filesystem import - Ping
                                                'Filesystem', 
                                                $filePath,
